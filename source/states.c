 ...
 void states(void){
    ...
    while(1){
        ...
        case IDLE:
            ...
            if(get_ordered_above(current_floor)>current_floor){
                current_state = MOVE_UP;
            }

            else if(get_ordered_above(current_floor)==current_floor){
                current_floor=DOOR_OPEN;
            }

            else if(get_ordered_below(current_floor)<current_floor){
        ...
        case MOVE_DOWN:
            ...
            if(queue_matrix[current_floor][HARDWARE_ORDER_INSIDE]==1){
                ...
                if(!(get_ordered_below(current_floor)<current_floor)){
                    motor_direction = motor_up;
                    hardware_command_movement(HARDWARE_MOVEMENT_STOP);
                    delete_order(current_floor,HARDWARE_ORDER_UP);
                    current_state=DOOR_OPEN;
